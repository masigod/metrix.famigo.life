// Netlify Function to proxy Airtable API requests for filtered table
...
exports.handler = async (event, context) => {
  ...
  // Get environment variables for the filtered table
  const API_KEY = process.env.Airtable_API_Key;
  const BASE_ID = process.env.Airtable_Base_ID;
  const TABLE_ID = process.env.Airtable_MetrixTable2_ID; // New table for filtered data
  ...
  try {
    ...
    console.error('Error in Airtable filter function:', error);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({
